...
function loadRecepients(toSearch=''){
    ...
    if(storedNames != null){
        for (let i = 0; i < storedNames.length; i++) {
            if(storedNames[i].name.includes(toSearch) || 
               storedNames[i].publicKey.includes(toSearch) || 
               toSearch==''){
...
function addRecepient() {
    Swal.fire({
        title: 'Enter the name of the new recipient',
        input: 'text',
        inputLabel: 'name',
        showCancelButton: true,
        inputValidator: (value) => {
            ...
            if(!value || value === "") {
                ...
                for(let i = 0; i < editArray.length; i++){
                    ...
                    if(editArray[i].name == toEdit){
                        editArray[i].name = value;
                    }
                }
                localStorage.setItem("names",JSON.stringify(editArray));
                localStorage.setItem(value,localStorage.getItem(toEdit));
